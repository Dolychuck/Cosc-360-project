	...
	if($_SERVER["REQUEST_METHOD"] == "POST") {
		...
		if(isset($firstname) && isset($lastname) && isset($username) && isset($passwordOut)) {
			...
			if($error != null) {
				...
				$userID = -1;
				if($duplicate == false) {
					$sql2 = "INSERT INTO users (username,firstName,lastName,email,password,country,city,aboutme,status)VALUES (?,?,?,?,?,?,?,?,?);";
					if($statement = mysqli_prepare($connection, $sql2)) {
						mysqli_stmt_bind_param($statement, 'sssssssss', $username,$firstname,$lastname,$email,$passwordOut,$country,$city,$aboutme,$enabled);
